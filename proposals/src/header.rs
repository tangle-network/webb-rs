...
impl Display for TypedChainId {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            TypedChainId::Evm(id) => write!(f, "Evm({id})"),
            TypedChainId::Substrate(id) => write!(f, "Substrate({id})"),
            TypedChainId::PolkadotParachain(id) => {
                write!(f, "PolkadotParachain({id})")
            }
            TypedChainId::KusamaParachain(id) => {
                write!(f, "KusamaParachain({id})")
            }
            TypedChainId::RococoParachain(id) => {
                write!(f, "RococoParachain({id})")
            }
            TypedChainId::Cosmos(id) => write!(f, "Cosmos({id})"),
            TypedChainId::Solana(id) => write!(f, "Solana({id})"),
            TypedChainId::Ink(id) => write!(f, "Ink({id})"),
        }
    }
}
