...
///
/// The [`FungibleAssetAddProposal`] allows the token specified by the `AssetId` to
...
impl From<FungibleAssetAddProposal> for Vec<u8> {
    fn from(proposal: FungibleAssetAddProposal) -> Self {
        proposal.into_bytes()
    }
}

impl TryFrom<Vec<u8>> for FungibleAssetAddProposal {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        let proposal = FungibleAssetAddProposal {
            header,
            name,
            asset_id,
        };
...
mod tests {
    ...
    fn encode() {
        ...
        let proposal = FungibleAssetAddProposal::builder()
            .header(header)
            .name("test".to_string())
            .asset_id(1)
            .build();
    ...
    fn decode() {
        ...
        let proposal =
            FungibleAssetAddProposal::try_from(proposal_bytes.to_vec())
                .unwrap();
        let target = SubstrateTargetSystem::builder()
            .pallet_index(35)
            .tree_id(2)
