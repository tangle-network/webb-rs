...
impl Drop for AnvilInstance {
    fn drop(&mut self) {
        ...
        // we need to send a SIGTERM to the anvil process.
        //
        // On Windows, this is not possible, so we just kill the process.
        #[cfg(windows)]
        self.pid.kill().expect("could not kill anvil");
        // On Unix, we can send a SIGTERM.
        #[cfg(unix)]
        {
            let pid = self.pid.id();
            nix::sys::signal::kill(
...
impl Anvil {
    ...
    pub fn spawn(self) -> AnvilInstance {
        ...
        let stdout = child
            .stdout
            .take()
            .expect("Unable to get stdout for anvil child process");
        ...
        loop {
            if start
                + Duration::from_millis(
                    self.timeout.unwrap_or(ANVIL_STARTUP_TIMEOUT_MILLIS),
                )
                <= Instant::now()
            {
                panic!(
                    "Timed out waiting for anvil to start. Is anvil installed?"
                )
            }
            ...
            reader
                .read_line(&mut line)
                .expect("Failed to read line from anvil process");
            ...
            if is_private_key && line.starts_with('(') {
                let key_str = line
                    ...
                    .last()
                    .unwrap_or_else(|| {
                        panic!("could not parse private key: {}", line)
                    })
                    .trim();
                let key_hex =
                    hex::decode(key_str).expect("could not parse as hex");
                let key = K256SecretKey::from_bytes(
                    &GenericArray::clone_from_slice(&key_hex),
                )
