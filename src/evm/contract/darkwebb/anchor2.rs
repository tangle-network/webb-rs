pub use anchor2contract_mod::*;
#[allow(clippy::too_many_arguments)]
mod anchor2contract_mod {
    ...
    impl<'a, M: ethers::providers::Middleware> Anchor2Contract<M> {
        ...
        pub fn events(
            &self,
        ) -> ethers::contract::builders::Event<M, Anchor2ContractEvents>
        {
    ...
    pub enum Anchor2ContractEvents {
    ...
    impl ethers::core::abi::Tokenizable for Anchor2ContractEvents {
        fn from_token(
            ...
            Self: Sized,
        {
            if let Ok(decoded) = DepositFilter::from_token(token.clone()) {
                return Ok(Anchor2ContractEvents::DepositFilter(decoded));
            }
            if let Ok(decoded) = EdgeAdditionFilter::from_token(token.clone()) {
                return Ok(Anchor2ContractEvents::EdgeAdditionFilter(decoded));
            }
            if let Ok(decoded) = EdgeUpdateFilter::from_token(token.clone()) {
                return Ok(Anchor2ContractEvents::EdgeUpdateFilter(decoded));
            }
            if let Ok(decoded) =
                RootHistoryRecordedFilter::from_token(token.clone())
            {
                return Ok(Anchor2ContractEvents::RootHistoryRecordedFilter(
                    decoded,
                ));
            }
            if let Ok(decoded) =
            ...
            {
                return Ok(Anchor2ContractEvents::RootHistoryUpdateFilter(
                    decoded,
                ));
            }
            if let Ok(decoded) = WithdrawalFilter::from_token(token.clone()) {
                return Ok(Anchor2ContractEvents::WithdrawalFilter(decoded));
            }
        ...
        fn into_token(self) -> ethers::core::abi::Token {
            match self {
                Anchor2ContractEvents::DepositFilter(element) => {
                    element.into_token()
                }
                Anchor2ContractEvents::EdgeAdditionFilter(element) => {
                    element.into_token()
                }
                Anchor2ContractEvents::EdgeUpdateFilter(element) => {
                    element.into_token()
                }
                Anchor2ContractEvents::RootHistoryRecordedFilter(element) => {
                    element.into_token()
                }
                Anchor2ContractEvents::RootHistoryUpdateFilter(element) => {
                    element.into_token()
                }
                Anchor2ContractEvents::WithdrawalFilter(element) => {
                    element.into_token()
                }
            }
        }
    }
    impl ethers::core::abi::TokenizableItem for Anchor2ContractEvents {}
    impl ethers::contract::EthLogDecode for Anchor2ContractEvents {
        fn decode_log(
