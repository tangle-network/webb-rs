...
pub mod multi_asset_v_anchor_tree_contract {
    ...
    pub struct MultiAssetVAnchorTreeContract<M>(
    ...
    impl ::ethers::contract::EthLogDecode for MultiAssetVAnchorTreeContractEvents {
        fn decode_log(
            ...
            if let Ok(decoded) = EdgeAdditionFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::EdgeAdditionFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = EdgeUpdateFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::EdgeUpdateFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = InsertionFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::InsertionFilter(
                        decoded,
                    ),
                );
            }
            ...
            if let Ok(decoded) = NewCommitmentFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::NewCommitmentFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = NewNullifierFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::NewNullifierFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = PublicKeyFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::PublicKeyFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = SetHandlerFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::SetHandlerFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = SetVerifierFilter::decode_log(log) {
                return Ok(
                    MultiAssetVAnchorTreeContractEvents::SetVerifierFilter(
                        decoded,
                    ),
                );
            }
