...
impl core::fmt::Display for CircomError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            CircomError::InvalidVerifyingKeyBytes => {
                write!(f, "Invalid verifying key bytes")
            }
            CircomError::InvalidProofBytes => write!(f, "Invalid proof bytes"),
            CircomError::InvalidBuilderConfig => {
                write!(f, "Invalid builder config")
            }
            ...
            CircomError::ParameterGenerationFailure => {
                write!(f, "Parameter generation failure")
            }
        }
    }
}
...
impl Proof {
    pub fn decode(input: &[u8]) -> Result<Self, Error> {
        ...
        let a = decoded[0]
            .clone()
            .into_fixed_array()
            .ok_or(CircomError::InvalidProofBytes)?;
        let a_x = a[0]
            .clone()
            .into_uint()
            .ok_or(CircomError::InvalidProofBytes)?;
    ...
    pub fn encode(&self) -> Result<Vec<u8>, Error> {
        ...
        let encoded = encode(&[Token::Tuple(vec![
            Token::FixedArray(vec![Token::Uint(a_x), Token::Uint(a_y)]),
            Token::FixedArray(vec![
                Token::FixedArray(vec![Token::Uint(b_x_1), Token::Uint(b_x_0)]),
                Token::FixedArray(vec![Token::Uint(b_y_1), Token::Uint(b_y_0)]),
            ]),
            Token::FixedArray(vec![Token::Uint(c_x), Token::Uint(c_y)]),
        ])]);
        Ok(encoded)
    }
}
...
fn u256_to_point<F: PrimeField>(point: U256) -> Result<F, Error> {
    ...
    let bigint = F::BigInt::read(&buf[..])
        .map_err(|_| CircomError::InvalidProofBytes)?;
